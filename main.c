...
void callFunction(char *funcName, char **args, int argCount) {
    ...
    if (strcmp(funcName, "testingParser") == 0) {
        ...
        if (argCount >= 1) {
            User *user = search_user_by_name(args[0]);
            if (user != NULL) {
                print_friends(user);
            } else {
        ...
        // Uncomment and implement these functions when ready
...
} // Add this closing brace to properly close the callFunction definition
